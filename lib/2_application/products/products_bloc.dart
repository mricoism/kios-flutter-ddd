        ...
        await events.when(
          ...
          nextPage: (thisProductItem) async {
            ...
            Either<ProductFailure, ProductSearch> either = response.match((l) {
              debugPrint(
                  'flow productBloc Event nextPage fail to get data (response.match)');
              return left(l);
            }, (r) {
              emit(state.copyWith(isMoreLoading: false));
              debugPrint(
                  'flow productBloc Event nextPage success to load data');
            ...
            response.fold((_) {
              emit(state.copyWith(isMoreLoading: false));
              debugPrint(
                  'flow productBloc Event nextPage fail to get data (response.fold)');
            }, (r) {
              debugPrint(
                  'flow productBloc Event nextPage success to load data');
              var newList = List<ProductItem>.from(thisProductItem)..addAll(r);
          ...
          search: (String keyword) async {
            ...
            emit(state.copyWith(
                optionFailureOrSuccess: optionOf(either), check: keyword));
          },
        );
      },
